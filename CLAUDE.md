...
# OR manually add repository: https://github.com/0ofta/SFPLiberate
```
...
- **Known endpoints:** `GET /stats` (battery/uptime/signal), `GET /xsfp/module/details` (module presence/vendor/part number), `GET /xsfp/module/start` + `GET /xsfp/module/data` (read EEPROM - verified against real hardware), `PUT /api/v1/modules/{id}` (backend-only: replace a saved module's EEPROM data, e.g. to edit its serial before a test write), `POST /xsfp/sync/start` + `POST /xsfp/sync/data` (write EEPROM - **verified against real hardware, with a caveat**: this only stages data into the device's internal snapshot buffer. **It does not write to the physical module.** Applying the snapshot requires physically confirming on the SFP Wizard's own screen - there is no BLE/API command to do this remotely, confirmed by exhaustively searching sfpw-tool's source (grepped for apply/confirm/flash/commit) and finding every write path ends with "use the device screen to apply snapshot to module." This is almost certainly a deliberate hardware safety gate, not a missing feature.
